...
RSpec.describe 'Merchants Revenue Show API' do
  context 'has merchants' do
    ...
    let!(:trans7) { create :transaction, { result: 'failed', invoice_id: invoice1.id } }
      ...
      it 'returns result of a specific merchants revenue' do
        ...
        expect(merchants[:data][:id]).to eq("#{merchant.id}")
        expect(merchants[:data][:attributes][:revenue]).to eq(850.0)
      end
